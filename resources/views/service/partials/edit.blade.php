<div class="p-4">
    ...
    <div class="card shadow-sm border-0">
        ...
        <form method="POST" action="{{ route('service.update', $client->id) }}">
            @csrf
            @method('PUT')
                    ...
                    <div class="beneficiary-grid">
                        ...
                        @if($b)
                            <button
                                ...
                                data-url="{{ url('service/' . $client->id . '/beneficiaries/' . $b->id) }}"
                                onclick="deleteServiceContactRow(this)"
                    ...
                    <div class="emergency-grid">
                        ...
                        @if($e)
                            <button
                                type="button"
                                class="contact-delete-btn"
                                data-url="{{ url('service/' . $client->id . '/emergency/' . $e->id) }}"
                                onclick="deleteServiceContactRow(this)"
            ...
            <div class="text-end">
                <button class="btn-gold btn-lg">Save Changes</button>
            </div>
...
<script>
const SERVICE_CSRF = '{{ csrf_token() }}';

function deleteServiceContactRow(button) {
    const url = button.dataset.url;
    const row = button.closest('.beneficiary-row, .emergency-row');
